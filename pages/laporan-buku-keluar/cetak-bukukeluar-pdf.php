...
$pdf->Cell(0, 7, 'LAPORAN BUKU KELUAR', 0, 1, 'C');
...
if (!empty($_GET["dari_tanggal"]) && empty($_GET["sampai_tanggal"]))
    $kondisi = "where tanggal_pinjam='" . $_GET['dari_tanggal'] . "' ";
if (!empty($_GET["dari_tanggal"]) && !empty($_GET["sampai_tanggal"]))
    $kondisi = "where tanggal_pinjam between '" . $_GET['dari_tanggal'] . "' and '" . $_GET['sampai_tanggal'] . "'";
...
while ($data = mysqli_fetch_array($hasil)):
    ...
    $pdf->Cell(20, 6, substr($data['nama_penulis'], 0, 19), 1, 0);
    $pdf->Cell(20, 6, substr($data['nama_penerbit'], 0, 19), 1, 0);
    $pdf->Cell(20, 6, substr($data['nama_supplier'], 0, 19), 1, 0);
    $pdf->Cell(20, 6, $data['jml'], 1, 1);
    $no++;
endwhile;
